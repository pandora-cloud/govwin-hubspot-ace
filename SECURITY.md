...
- The internet-reachable HubSpot UI Extension callbacks are split into two Lambdas. The reads Lambda (`<prefix>-ui-ext-reads`) IAM scope is `partnercentral:ListSolutions` + the webhook signing-secret read; it carries no HubSpot private app token, no DynamoDB reads, and no Partner Central writes. The writes Lambda (`<prefix>-ui-ext-writes`) carries the update path but NOT `CreateOpportunity` or `StartEngagementFromOpportunityTask` — those stay on the trusted shared role behind the SQS pipeline. A parser or dependency CVE on the reads surface cannot escalate to the update path.
- Property-change webhooks for `govwin_aws_cosell_id` fire a real-time SNS audit alert when the `changeSource` is anything other than `INTEGRATION` (i.e., a BD hand-edit of an AWS-owned identifier). The runtime self-heal verify in `update_in_ace` refuses to mutate AWS if the deal's recovered ACE id resolves to a foreign `PartnerOpportunityIdentifier`; the SNS audit alert lets the operator reconcile before the next save trips the verify.
