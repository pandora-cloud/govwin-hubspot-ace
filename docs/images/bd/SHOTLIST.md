...
| `update-closed-lost.png` | The Update form's **LifeCycle** section with the **Stage** dropdown set to **Closed Lost** and the **Closed Lost reason** dropdown showing. Capture on a live (not yet terminal) deal; the form is disabled on terminal deals. | The locked GovWin ID and AWS opportunity ID at the top of the form. |
| `card-closed-lost.png` | The card on a deal that has reached a terminal **Closed Lost** state: the status badge reads **Closed lost**, the Update button is greyed out, and the AWS opportunity identifier is locked at the top of the card. Best captured on an existing test deal that has already been closed. | The AWS opportunity ID, GovWin ID, associated company name. |
